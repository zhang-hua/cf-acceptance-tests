...
var _ = Describe("Process Types", func() {
	...
	Describe("Staging", func() {
		...
		Describe("with a procfile", func() {
			...
			It("allows overriding the web process type with a manually-specified start command", func() {
				Expect(cf.Cf("push", appName, "-p", helpers.NewAssets().NodeWithProcfile, "-c", "node *.js").Wait(CF_PUSH_TIMEOUT)).To(Exit(0))

				var appsResponse AppsResponse
				cfResponse := cf.Cf("curl", fmt.Sprintf("/v2/apps?q=name:%s", appName)).Wait(DEFAULT_TIMEOUT).Out.Contents()
				json.Unmarshal(cfResponse, &appsResponse)

				Expect(appsResponse.Resources[0].Entity.DetectedStartCommand).To(Equal("node *.js"))
			})
		})
	})
})
